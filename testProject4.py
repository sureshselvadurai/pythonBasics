...
class TestServer(unittest.TestCase):
    ...
    def test_reviews_ok_Info1(self):
        ...
        self.assertEqual(review0['reviewText'],
        ...
        self.assertEqual(review1['reviewText'],
    ...
    def test_reviews_missingNum(self):
        ...
        r = requests.get(url)
        body = r.json()
        self.assertEqual(body['num'], 'invalid')
        self.assertEqual(body['success'], False)

    # Test /index endpoint ------------------------------------------------------
    # Tests a get request to /index?word=nice&num=2 and asserts that the response is HTTP/1.1 200 OK
    def test_index_ok_httpstatus(self):
        url = "http://" + self.host + ":" + self.port + "/index?word=nice&num=2"
        r = requests.get(url)
        self.assertEqual(r.status_code, 200)

    # Tests a get request to /index?word=nice&num=2 and checks returned json
    def test_index_ok_Info1(self):
        url = "http://" + self.host + ":" + self.port + "/index?word=nice&num=2"
        r = requests.get(url)
        self.assertEqual(r.status_code, 200)
        body = r.json()
        self.assertEqual(len(body['reviews']), 2)
        self.assertEqual(body['word'], 'nice')
        reviews = body['reviews']
        review0 = reviews[0]
        review1 = reviews[1]
        self.assertEqual('nice' in review0['reviewText'].lower(), True)
        self.assertEqual('nice' in review1['reviewText'].lower(), True)
        self.assertEqual(body['success'], True)

    # Tests a get request to /index?word=nice&num=3 and checks returned json
    def test_index_ok_reviews3(self):
        url = "http://" + self.host + ":" + self.port + "/index?word=nice&num=3"
        r = requests.get(url)
        self.assertEqual(r.status_code, 200)
        body = r.json()
        ...
        self.assertEqual('nice' in review0['reviewText'].lower(), True)
        self.assertEqual('nice' in review1['reviewText'].lower(), True)
        self.assertEqual('nice' in review2['reviewText'].lower(), True)
        self.assertEqual(body['success'], True)

    def test_index_missingParameters(self):
        url = "http://" + self.host + ":" + self.port + "/index"
        r = requests.get(url)
        body = r.json()
        self.assertEqual(body['success'], False)
        self.assertEqual(body['word'], 'invalid')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("usage: testProject4.py <host> <port>")
        sys.exit()
    ...
    # Testing /index
    suite.addTest(TestServer("test_index_ok_httpstatus", host, port))
    suite.addTest(TestServer("test_index_ok_Info1", host, port))
    suite.addTest(TestServer("test_index_ok_reviews3", host, port))
    suite.addTest(TestServer("test_index_missingParameters", host, port))
